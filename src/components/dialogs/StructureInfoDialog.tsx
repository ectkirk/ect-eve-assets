...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <h4 className="text-xs font-semibold text-content-secondary uppercase tracking-wider">{title}</h4>
...
function ServiceBadge({ name, state }: { name: string; state: 'online' | 'offline' | 'cleanup' }) {
  ...
  return (
    ...
    >
      <span className={cn('w-1.5 h-1.5 rounded-full', isOnline ? 'bg-status-positive' : 'bg-content-muted')} />
      {name}
    </span>
  )
}
...
const STATE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  armor_vulnerable: { label: 'Armor Vulnerable', color: 'text-status-highlight' },
...
export function StructureInfoDialog({ open, onOpenChange, structure, ownerName }: StructureInfoDialogProps) {
  ...
  const stateInfo = STATE_LABELS[structure.state] ?? { label: 'Unknown', color: 'text-content-muted' }
      ...
      <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <DialogTitle className="text-lg">{structure.name || 'Unnamed Structure'}</DialogTitle>
        ...
        <div className="space-y-4 mt-2">
          <Section title="Location">
            <InfoRow label="System" value={systemName} className="text-status-info" />
          ...
          <Section title="Status">
            ...
            {structure.state_timer_start && (
              <InfoRow label="State Started" value={formatDateTime(structure.state_timer_start)} />
            )}
            {structure.state_timer_end && (
              <InfoRow
                label="State Ends"
                value={formatDateTime(structure.state_timer_end)}
                className={structure.state.includes('reinforce') ? 'text-status-negative' : undefined}
              />
            )}
            {structure.unanchors_at && (
              <InfoRow label="Unanchors At" value={formatDateTime(structure.unanchors_at)} className="text-status-highlight" />
            )}
          </Section>

          <Section title="Reinforcement">
            <InfoRow label="Vulnerability Window" value={formatReinforceHour(structure.reinforce_hour)} />
            {structure.next_reinforce_hour !== undefined && structure.next_reinforce_apply && (
              <>
                <InfoRow label="Pending Window" value={formatReinforceHour(structure.next_reinforce_hour)} className="text-status-info" />
                <InfoRow label="Change Applies" value={formatDateTime(structure.next_reinforce_apply)} />
              </>
            )}
          </Section>
          ...
          <Section title="Fuel">
            <InfoRow
              label="Fuel Expires"
              value={structure.fuel_expires ? formatDateTime(structure.fuel_expires) : 'No fuel data'}
              className={structure.fuel_expires ? undefined : 'text-content-muted'}
            />
          </Section>
          ...
          {(!structure.services || structure.services.length === 0) && (
            <Section title="Services">
              <div className="py-2 text-content-muted text-sm">No services installed</div>
            </Section>
          )}
        </div>
      </DialogContent>
