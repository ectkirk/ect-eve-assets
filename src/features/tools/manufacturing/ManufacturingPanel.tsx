...
import { ManufacturableItemTree } from './ManufacturableItemTree'
...
export function ManufacturingPanel() {
  ...
  const manufacturableTypeIds = useMemo(() => {
    const ids = new Set<number>()
    for (const type of types.values()) {
      if (type.categoryId === BLUEPRINT_CATEGORY_ID && type.productId) {
        ids.add(type.productId)
      }
    }
    return ids
  }, [types])
  ...
  const searchResults = useMemo(() => {
    ...
    return matched
  }, [debouncedQuery, types, allTypesLoaded, manufacturableTypeIds])
        ...
        <div className="flex w-72 flex-shrink-0 flex-col overflow-hidden border-r border-border">
          <div className="border-b border-border p-2">
            <div ref={searchContainerRef} className="relative">
              ...
              <input
                ...
                placeholder={
                  isLoading ? t('manufacturing.loadingTypes') : t('common:search.placeholder')
                }
                disabled={isLoading}
              ...
              {dropdownOpen && searchResults && searchResults.length > 0 && (
                <div className="absolute left-0 right-0 z-20 mt-1 max-h-80 overflow-y-auto rounded border border-border bg-surface-secondary shadow-lg">
                  {searchResults.map((type) => (
                    ...
                    >
                      <TypeIcon typeId={type.id} categoryId={type.categoryId} size="md" />
                      <div>
          ...
          <div className="min-h-0 flex-1">
            {isLoading ? (
              ...
              <ManufacturableItemTree
                categories={categories}
                groups={groups}
                types={types}
                manufacturableTypeIds={manufacturableTypeIds}
        ...
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          {selectedType ? (
            ...
            <div className="flex h-full items-center justify-center text-content-secondary">
              <p>{t('manufacturing.selectItemPrompt')}</p>
            </div>
          )}
        </div>
      </div>
