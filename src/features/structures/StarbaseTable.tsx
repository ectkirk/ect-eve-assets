...
export function StarbaseTable({
  ...
  const sortedRows = useMemo(() => {
    const getValue = (
      row: StarbaseRow,
      column: StarbaseSortColumn
    ): number | string => {
      switch (column) {
        case 'name':
          return (
            row.moonName ?? `Moon ${row.starbase.moon_id ?? 0}`
          ).toLowerCase()
        case 'type':
          ...
          {sortedRows.map((row) => {
            ...
            return (
              <ContextMenu key={`pos-${row.starbase.starbase_id}`}>
                ...
                <ContextMenuContent>
                  <ContextMenuItem
                    onClick={() => onViewPosInfo(row.starbase, row.ownerName)}
                  >
                    Show POS Info
                  </ContextMenuItem>
                </ContextMenuContent>
