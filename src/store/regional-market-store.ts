...
function hydrateFromRecords(
  ...
  for (const record of prices) {
    if (record.lowestPrice !== null) pricesByType.set(record.typeId, record.lowestPrice)
    if (record.highestBuyPrice !== null) buyPricesByType.set(record.typeId, record.highestBuyPrice)
  ...
  return { pricesByType, pricesByLocation, buyPricesByType, buyPricesByLocation, lastFetchAt, trackedTypes, trackedStructures }
}

function collectTasks(
  state: RegionalMarketState,
  now: number
): { regionalTasks: RegionalTask[]; structureTasks: StructureTask[]; earliestExpiry: number } {
  ...
  for (const [structureId, { characterId, typeIds, lastFetchAt }] of state.trackedStructures) {
...
export const useRegionalMarketStore = create<RegionalMarketStore>((set, get) => ({
  ...
  update: async () => {
    ...
    const { regionalTasks, structureTasks, earliestExpiry } = collectTasks(state, Date.now())
    ...
    try {
      ...
      if (get().trackedTypes.size > 0 || get().trackedStructures.size > 0) {
        useExpiryCacheStore.getState().setExpiry(OWNER_KEY, ENDPOINT_PATTERN, Date.now() + CACHE_TTL_MS)
      }
    } catch (err) {
      set({ isUpdating: false, updateError: err instanceof Error ? err.message : 'Unknown error' })
    }
  },
  ...
  untrackTypes: async (typeIds) => {
    ...
    if (keysToDelete.length > 0) {
      ...
      try {
        ...
        logger.info('Untracked types from regional market', { module: 'RegionalMarketStore', types: typeIds.length })
      } catch (err) {
  ...
  registerStructures: (structureIds, typeIds, characterId) => {
    ...
    for (const structureId of structureIds) {
      ...
      if (existing) {
        ...
        hasChanges = true
        newStructures.push({ structureId, characterId, typeIds: Array.from(typeIdSet), lastFetchAt: 0 })
      }
    }
  ...
  untrackStructures: async (structureIds) => {
    ...
    cleanupLocationPrices(pricesByLocation, pricesByType, structureIdSet, Math.min)
    cleanupLocationPrices(buyPricesByLocation, buyPricesByType, structureIdSet, Math.max)
    ...
    if (toDelete.length > 0) {
      ...
      try {
        ...
        logger.info('Untracked structures from regional market', { module: 'RegionalMarketStore', structures: toDelete.length })
      } catch (err) {
  ...
  removeForOwner: async (ownerType, ownerId) => {
    ...
    if (structuresToRemove.length > 0) {
      ...
      logger.info('Removed structures for owner', { module: 'RegionalMarketStore', characterId: ownerId, structures: structuresToRemove.length })
    }
  ...
  getPriceAtLocation: (typeId, locationId) => get().pricesByLocation.get(typeId)?.get(locationId),
  getHighestBuyAtLocation: (typeId, locationId) => get().buyPricesByLocation.get(typeId)?.get(locationId),
