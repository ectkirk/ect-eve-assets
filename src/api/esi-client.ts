...
class ESIClient {
  ...
  async fetchWithMeta<T>(
    ...
    const executor = async (): Promise<ESIResponseMeta<T>> => {
      ...
      const cached = this.cache.get(cacheKey)
      if (cached && Date.now() < cached.expires) {
        logger.debug('ESI cache hit (with meta)', { module: 'ESI', url, expiresIn: cached.expires - Date.now() })
        return {
      ...
      const etagToUse = providedEtag ?? cached?.etag
      if (etagToUse) {
        headers['If-None-Match'] = etagToUse
      }

      logger.debug('ESI request (with meta)', { module: 'ESI', url, method: fetchOptions.method ?? 'GET' })
      ...
      if (!response.ok) {
        ...
        if (response.status === 403 || response.status === 404) {
          logger.debug('ESI request returned expected error', { module: 'ESI', endpoint, status: response.status })
        } else {
          logger.error('ESI request failed', undefined, { module: 'ESI', endpoint, status: response.status })
        }
        throw new ESIError(errorMessage, response.status)
      ...
      if (responseEtag) {
        this.cache.set(cacheKey, { data, etag: responseEtag, expires: expiresAt })
      }
  ...
  async fetchWithPaginationMeta<T>(
    ...
    let page = 1
    let hasMore = true
    let latestExpiresAt = Date.now() + DEFAULT_CACHE_MS
    ...
    while (hasMore) {
      ...
      const expiresHeader = response.headers.get('Expires')
      if (expiresHeader) {
        latestExpiresAt = new Date(expiresHeader).getTime()
      }
      const responseEtag = response.headers.get('ETag')
      if (responseEtag) {
